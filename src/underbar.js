(function () {
  ...
  _.uniq = function (array, isSorted, func) {
    var result = [];
    var funcArr = [];
    if (func === undefined) {
      func = _.identity;
    ...
    _.each(array, function (val) {
      var funcResult = func(val);
      if (isSorted) {
        if (funcArr.includes(funcResult) === false) {
          funcArr.push(funcResult);
          result.push(val);
        }
      } else if (result.includes(funcResult) === false) { // isSorted = false
  ...
  _.every = function (collection, iterator) {
    ...
    for (var i = 0; i < collection.length; i++) {
      if (Boolean(iterator(collection[i])) === false) {
        return false;
      }
    }
    return true;
  ...
  _.memoize = function (func) {
    ...
    return function () {
      if (!resultObj[func + JSON.stringify(arguments)]) {

        resultObj[func + JSON.stringify(arguments)] = func.apply(this, arguments);
      }

      return resultObj[func + JSON.stringify(arguments)];
    };
  };
  ...
  _.invoke = function (collection, functionOrKey, args) {
    ...
    _.each(collection, function(val) {
      if (typeof functionOrKey === 'function') {
        ...
        result.push(val[functionOrKey].apply(val, args));
      }
    });
